...
public class Docgen extends STServiceAdapter {
    ...
    public static final Comparator<AnnotatedValue<Resource>> ANNOTATED_VALUES_SHOW_COMPARATOR;
    ...
    public OWLDocumentation buildOWLDocumentation() throws Exception {
        ...
        Literal emptyLiteral = SimpleValueFactory.getInstance().createLiteral("");
        ...
        Map<RDFResourceRole, List<AnnotatedValue<Resource>>> role2resources = resourceMap.values().stream().collect(groupingBy(av -> STServiceAdapter.getRoleFromNature(av.getAttributes().getOrDefault("nature", emptyLiteral).stringValue()), Collectors.toList()));
        Set<Resource> locallyDefinedResources = resourceMap.values().stream().filter(av -> STServiceAdapter.getGraphFromNature(av.getAttributes().getOrDefault("nature", emptyLiteral).stringValue()).map(getWorkingGraph()::equals).orElse(false)).map(AnnotatedValue::getValue).collect(toSet());
        resourceMap.values().forEach(av -> { if (locallyDefinedResources.contains(av.getValue())) av.getAttributes().put("localIRI", Values.literal(true)); });
    ...
    public static class OWLDocumentation {
