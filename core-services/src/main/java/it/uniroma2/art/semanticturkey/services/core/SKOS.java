...
public class SKOS extends STServiceAdapter {
    ...
    /**
     * @param newConcept            IRI of the new created concept. If not provided a random IRI is generated.
     * @param label                 preferred label of the concept
     * @param broaderConcept        broader of the new created concept. If not provided the new concept will be a top concept
     * @param conceptSchemes        concept schemes where the concept belongs
     * @param conceptCls            class type of the new created concept. It must be a subClassOf skos:Concept. If not provided the new concept
    ...
    public AnnotatedValue<IRI> createConcept(
        ...
        modelAdditions.add(newConcept, RDF.TYPE, conceptCls);

        for (IRI conceptScheme : conceptSchemes) {
            modelAdditions.add(newConcept, org.eclipse.rdf4j.model.vocabulary.SKOS.IN_SCHEME, conceptScheme);//?conc skos:inScheme ?sc
        }

        if (broaderConcept != null) {//?conc skos:broader ?broad
            //check if the client passed a broaderProp , otherwise, set it as skos:broader
            broaderProp = getHierachicalProp(broaderProp);
            modelAdditions.add(newConcept, broaderProp, broaderConcept);
        } else { //?conc skos:topConceptOf ?sc
            for (IRI conceptScheme : conceptSchemes) {
                modelAdditions.add(newConcept, org.eclipse.rdf4j.model.vocabulary.SKOS.TOP_CONCEPT_OF, conceptScheme);
            }
        }
        ...
        if (xLabelIRI != null) {
            ResourceLevelChangeMetadataSupport.currentVersioningMetadata().addCreatedResource(xLabelIRI, RDFResourceRole.xLabel);
        }
    ...
    public AnnotatedValue<IRI> createConceptScheme(
        ...
        if (newScheme == null) {
            newScheme = generateConceptSchemeURI(label);
        }
        ...
        //set versioning metadata
        ResourceLevelChangeMetadataSupport.currentVersioningMetadata().addCreatedResource(newScheme, RDFResourceRole.conceptScheme);
        if (xLabelIRI != null) {
            ResourceLevelChangeMetadataSupport.currentVersioningMetadata().addCreatedResource(xLabelIRI, RDFResourceRole.xLabel);
        }
        ...
        AnnotatedValue<IRI> annotatedValue = new AnnotatedValue<>(newScheme);
    ...
    public Collection<AnnotatedValue<Literal>> getAltLabels(@LocallyDefined IRI concept) {
    ...
    public AnnotatedValue<Resource> createCollection(
        ...
        if (newCollection == null) {
            ...
            if (newCollection.equals(RDF.NIL)) {
                throw new IllegalArgumentException("Cannot create collection rdf:nil");
            }
        ...
        //set versioning metadata
        ResourceLevelChangeMetadataSupport.currentVersioningMetadata().addCreatedResource(newCollectionRes, RDFResourceRole.skosCollection);
        if (xLabelIRI != null) {
            ResourceLevelChangeMetadataSupport.currentVersioningMetadata().addCreatedResource(xLabelIRI, RDFResourceRole.xLabel);
        }
     ...
     * </ul>
     *
     * @param label   the preferred label accompanying the concept (can be <code>null</code>)
     * @param schemes the schemes to which the concept is being attached at the moment of its creation (can be
     *                <code>null</code>)
     * @return
     * @throws URIGenerationException
     */
     ...
     *
     * @param label the preferred label accompanying the concept scheme (can be <code>null</code>)
     * @return
     * @throws URIGenerationException
     */
     ...
     *
     * @param label the preferred label accompanying the collection (can be <code>null</code>)
     * @return
     * @throws URIGenerationException
     */
    ...
    /**
     * Run checks to verify if the given label can be created/assigned according {@code checkExistingPrefLabel}
     * and {@code checkPrefAltLabelClash}. In case a check fails, throws an exception.
     * Only if the lexicalization model is SKOSXL, creates and returns the created xLabel IRI.
     *
     * @param resource
     * @param label
     ...
     * @throws URIGenerationException
     */
    private IRI checkAndCreateLexicalization(Resource resource, Literal label, List<IRI> conceptSchemes,
            boolean checkExistingPrefLabel, boolean checkPrefAltLabelClash)
            throws UnsupportedLexicalizationModelException, PrefPrefLabelClashException, PrefAltLabelClashException, URIGenerationException {
        IRI lexModel = getProject().getLexicalizationModel();
        IRI xLabelIRI = null;
        if (lexModel.equals(Project.SKOS_LEXICALIZATION_MODEL)) {
            if (checkExistingPrefLabel) {
                checkIfAddPrefLabelIsPossible(getManagedConnection(), label, true, conceptSchemes);
            }
            if (checkPrefAltLabelClash) {
                checkIfPrefAltLabelClash(getManagedConnection(), label);
            }
    ...
    /**
     * Add the triples about lexicalization to {@code modelAdditions} according the lexicalization model.
     * @param resource
     * @param label
     * @param xLabelIRI
     ...
     * parameters.
     *
     * @param lexicalForm            the textual content of the label
     * @param lexicalizedResource    the resource to which the label will be attached to
     * @param lexicalizationProperty the property used for attaching the label
     * @return
     * @throws URIGenerationException
     */
