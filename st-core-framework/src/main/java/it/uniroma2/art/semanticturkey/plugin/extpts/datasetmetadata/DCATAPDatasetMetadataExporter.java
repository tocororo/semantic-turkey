...
/**
 * A {@link DatasetMetadataExporter} for the <a href="https://joinup.ec.europa.eu/asset/dcat_application_profile/asset_release/dcat-ap-v11">
 * Data Catalog Vocabulary (DCAT)</a> (<a href="https://joinup.ec.europa.eu/system/files/project/dcat-ap_.1.bmp"> bmp</a>)
 * 
 * @author <a href="mailto:turbati@info.uniroma2.it">Andrea Turbati </a>
 */
public class DCATAPDatasetMetadataExporter extends
		AbstractPlugin<DatasetMetadataExporterSettings, DCATAPDatasetMetadataExporterSettings, DCATAPDatasetMetadataExporterFactory>
		...
		try {
			try (RepositoryConnection metadataConnection = tempMetadataRepository.getConnection()) {
				...
				if(catalogue_publisherAgentType!=null && !catalogue_publisherAgentType.isEmpty()){
					...
					metadataModel.add(valueFactory.createStatement(catalogueAgentIRI, DCTERMS.TYPE, agent_type_iri));
				}	
				
				
				metadataModel.add(valueFactory.createStatement(catalogueIRI, DCTERMS.PUBLISHER, catalogueAgentIRI));
				...
				if(catalogue_homepage!=null && !catalogue_homepage.isEmpty()){
					...
					metadataModel.add(valueFactory.createStatement(catalogue_homepage_iri, RDF.TYPE, FOAF.DOCUMENT));
					metadataModel.add(valueFactory.createStatement(catalogueIRI, DCTERMS.TYPE, 
							catalogue_homepage_iri));
				}
				...
				if(catalogue_language!=null && !catalogue_language.isEmpty()){
					...
					metadataModel.add(valueFactory.createStatement(catalogue_language_iri, RDF.TYPE, DCTERMS.LINGUISTIC_SYSTEM));
				...
				if(catalogue_licence!=null && !catalogue_licence.isEmpty()){
					...
					metadataModel.add(valueFactory.createStatement(catalogue_licence_iri, RDF.TYPE, DCTERMS.LICENSE));
					metadataModel.add(valueFactory.createStatement(catalogueIRI, DCTERMS.LANGUAGE, 
				...
				if(catalogue_language!=null && !catalogue_language.isEmpty()){
					...
					metadataModel.add(valueFactory.createStatement(catalogue_themes_iri, RDF.TYPE, SKOS.CONCEPT_SCHEME));
					metadataModel.add(valueFactory.createStatement(catalogueIRI, DCTERMS.LANGUAGE, 
				...
				metadataModel.add(valueFactory.createStatement(datasetDistributionIRI,  RDF.TYPE, 
						DCAT.DISTRIBUTION));
				metadataModel.add(valueFactory.createStatement(datasetIRI, DCAT.HAS_DISTRIBUTION, 
						datasetDistributionIRI));
				IRI dataset_distributionAccessUrl = valueFactory.createIRI(pluginSettings.dataset_distributionAccessUrl);
				...
				if(dataset_distributionDescription!=null && !dataset_distributionDescription.isEmpty()){
					metadataModel.add(valueFactory.createStatement(datasetDistributionIRI, DCTERMS.DESCRIPTION, 
				...
				//Agent for dataset
				IRI dataset_publisherAgentIRI = valueFactory.createIRI(pluginSettings.dataset_publisherAgentIRI);
				metadataModel.add(valueFactory.createStatement(dataset_publisherAgentIRI, RDF.TYPE, FOAF.AGENT));
				...
				if(dataset_theme!=null && !dataset_theme.isEmpty()){
					...
					metadataModel.add(valueFactory.createStatement(dataset_theme_iri, RDF.TYPE, 
							SKOS.CONCEPT));
					metadataModel.add(valueFactory.createStatement(datasetIRI, DCAT.THEME, 
							dataset_theme_iri));
				}
