...
public class STServiceProcessorOLD extends AbstractProcessor {

	private static final String CLASSPATH_RESOURCES_BASE = "/" +STServiceProcessorOLD.class.getPackage().getName()
			.replace('.', '/');
	private static final String VELOCITY_PROPERTIES_CLASSPATH_NAME = CLASSPATH_RESOURCES_BASE
			+ "/velocityOLD.properties";
	private static final String TEMPLATE_CLASSPATH_CLASSPATH_NAME = CLASSPATH_RESOURCES_BASE
			+ "/spring_controllerOLD.vm";
		...
		for (Element i : roundEnv.getElementsAnnotatedWith(GenerateSTServiceController.class)) {
			...
			if (fqClassName != null) {
				try {
					...
					InputStream is = STServiceProcessorOLD.class
