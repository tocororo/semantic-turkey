...
function showAllProperties() {
	...
	if (sel.getAttribute("checked")) {
		...
		art_semanticturkey.HttpMgr
				.GETP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=property&request=getPropertiesTree");
	} else {
		...
		art_semanticturkey.HttpMgr
				.GETP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=individual&request=getIndDescription&instanceQName="
...
function onAccept() {
	...
	if (sindex == 0) {
		...
		AnnotFunctionList = annComponent.wrappedJSObject.getList();
		if (AnnotFunctionList[defaultAnnotFun] != null) {
			AnnotFunctionList[defaultAnnotFun][4](parameters, this);
		} else {
			var prompts = Components.classes["@mozilla.org/embedcomp/prompt-service;1"]
					.getService(Components.interfaces.nsIPromptService);
			prompts.alert(null, defaultAnnotFun
		...
		/*
		 * if (parameters.propType == "ObjectProperty" || parameters.propType ==
		 * "ObjectProperty_noexpl") { parameters.oncancel = false;
		 * window.openDialog(
		 * "chrome://semantic-turkey/content/enrichProperty.xul", "_blank",
		 * "modal=yes,resizable,centerscreen", parameters); // NScarpato
		 * 15/04/2008 if (parameters.oncancel == false) { close(); } } else if
		 * (parameters.propType == "AnnotationProperty" || parameters.propType ==
		 * "AnnotationProperty_noexpl") { parameters2 = new Object(); var lang =
		 * ""; parameters2.lang = lang; window.openDialog(
		 * "chrome://semantic-turkey/content/languageList.xul", "_blank",
		 * "modal=yes,resizable,centerscreen", parameters2); // NScarpato
		 * 19/06/2008 add lang parameters for annotation property
		 * art_semanticturkey.HttpMgr.GETP(
		 * "http://"+server+":1979/semantic_turkey/resources/stserver/STServer?service=annotation&request=relateAndAnnotate&instanceQName=" +
		 ...
		 * "&urlPage=" + encodeURIComponent(parameters.urlPage) + "&title=" +
		 * encodeURIComponent(parameters.pageTitle) + "&lang=" +
		 * encodeURIComponent(parameters2.lang) + "&op=bindCreate", false); //
		 * art_semanticturkey.HttpMgr.GETP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=annotation&instanceQName=" // +
		 * encodeURIComponent(parameters.subjectInstanceName) + //
		 * "&propertyQName=" //
		 * +encodeURIComponent(parameters.predicatePropertyName) + //
		 * "&objectQName=" + //
		 ...
		 * "&objectClsName=" + encodeURIComponent(parameters.objectClsName) // +
		 * "&urlPage=" + encodeURIComponent(parameters.urlPage) + // "&title=" + //
		 * encodeURIComponent(parameters.pageTitle)+"&op=bindCreate", // false); }
		 * else { art_semanticturkey.HttpMgr.GETP(
		 * "http://"+server+":1979/semantic_turkey/resources/stserver/STServer?service=annotation&request=relateAndAnnotate&instanceQName=" +
		 * encodeURIComponent(parameters.subjectInstanceName) +
		 * "&propertyQName=" +
		 * encodeURIComponent(parameters.predicatePropertyName) +
...
function getParameters(tree, index) {
	...
	for ( var i = 0; i < items.length; i++) {
		if (mytree.contentView.getIndexOfItem(items[i]) == index) {
			 ...
			 * art_semanticturkey.HttpMgr.GETP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=property&propertyQName=" +
