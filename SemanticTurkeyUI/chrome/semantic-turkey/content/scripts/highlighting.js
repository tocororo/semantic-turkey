...
function doHighlightPosition(bodyText, locationInPage, searchTerm,
		highlightStartTag, highlightEndTag, startOffset) {
	// the highlightStartTag and highlightEndTag parameters are optional
	if ((!highlightStartTag) || (!highlightEndTag)) {
		highlightStartTag = "<font style='color:blue; background-color:yellow;'>";
		highlightEndTag = "</font>";
	...
	/*
	 * addonOffset=0; i=0; l=0; alert(app); while(i<startOffset){
	 * if(app.indexOf("<") < startOffset+searchTerm.length){ c=app.charAt(l);
	 * l++; if(c!="<"){ alert("letto carattere "+c); i++; }else{ alert("letto
	 * carattere "+c); addonOffset+=app.indexOf(">")-app.indexOf("<");
	 * app=app.substr(app.indexOf(">")+1); l=0; alert("app addon\n"+app); }
	 * }else{ i=startOffset; } } alert("addonOffset"+addonOffset); //
	...
	alert(startPos);
	newText += bodyText.substring(0, startPos) + highlightStartTag
			+ bodyText.substr(startPos, searchTerm.length) + highlightEndTag
			+ bodyText.substr(startPos + searchTerm.length);
	return newText;
 ...
 */
/** NScarpato 28/05/2008 */
function highlightSearchTermsPosition(searchText, locationInPage,
		highlightStartTag, highlightEndTag, startOffset) {
	...
	bodyText = doHighlightPosition(bodyText, locationInPage, searchText,
			highlightStartTag, highlightEndTag, startOffset);
...
/*function findTextannotation(doc, annotation) {
	...
	for (var i = 0; i < childNodes.length; i++) {
		// art_semanticturkey.Logger.debug("name: " + childNodes[i].nodeName);
		if (childNodes[i].nodeType == 1) {
			findTextannotation(childNodes[i], annotation);
		}
		if (childNodes[i].nodeType == 3) {
			// art_semanticturkey.Logger.debug("value: " + childNodes[i].nodeValue);
			var tokens = childNodes[i].nodeValue.split(' ');
			for (var j = 0; j < tokens.length; j++) {
				if (tokens[j] != null) {
					if (regex.test(tokens[j])) {
						art_semanticturkey.Logger.debug("nodeValue: "
								+ childNodes[i].nodeValue);
						var temp = childNodes[i].nodeValue.split(tokens[j]);

						// art_semanticturkey.Logger.debug("length: " + temp.length);
						...
						if (k < temp.length) {
							while (k < temp.length) {
								art_semanticturkey.Logger.debug("temp: " + temp[k]);
								var newSpan = document.createElement("div");
