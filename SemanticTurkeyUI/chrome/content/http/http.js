...
/*
var mainLoader = Components.classes["@mozilla.org/moz/jssubscript-loader;1"].getService(Components.interfaces.mozIJSSubScriptLoader);

const log4jsberlURL = 'chrome://semantic-turkey/content/scripts/lib/log4js(berlios)/log4js-lib.js';
...
const log4jsURL = 'chrome://semantic-turkey/content/scripts/lib/log4js/log4js-mini.js';
mainLoader.loadSubScript(log4jsURL);

var httpLogger = new Log4js.getLogger("httpLogging");
httpLogger.setLevel(Log4js.Level.DEBUG);
httpLogger.addAppender(new Log4js.MozillaJSConsoleAppender());
...
/*function httpGetC(aURL, async, parameters) {
	if(NSIsSelected()== false){
	 	window.openDialog("chrome://semantic-turkey/content/initialize.xul","_blank","modal=yes,resizable,centerscreen");
	}
	if(isRepLoaded() == false && aURL.indexOf("repositoryChoice") == -1){
 ...
 * 
 * @param aURL
 * @param async true=asynchronous call, if invoked with only one parameter, it behaves as for the XMLHttpRequest.open() called with two parameters  
 * @return
 */
function httpGet(aURL, async,parameters) {
	_printToJSConsole("httpGet( "+aURL+") async:" + async + " parameters: " + parameters);	
	progressMeters=null;
...
httpReq.onreadystatechange = httpReadyStateChange;
//httpReq.addEventListener("readystatechange", httpReadyStateChange, false);
// QI it to nsIXMLHttpRequest to open and send the request:

httpReq.QueryInterface(Components.interfaces.nsIXMLHttpRequest);

if (typeof async == 'undefined') {
		_printToJSConsole("siamo in undefined");
		httpReq.open("GET", aURL);
	}
	else {
		_printToJSConsole("async è definito come " + async);
		httpReq.open("GET", aURL, async);
	}
	//FINO QUI*/
	try {
...
/**
 *@author NScarpato 02/04/2008
 * onProgress
 * add onProgress event for http request */
function httpProgress(e) {
    ...
    _printToJSConsole("% in on progress"+percentComplete+" scaricati "+e.position +" di "+e.totalSize);
...
/**NScarpato 19/03/2007 aggiunta funzione httpGetResultP funzione che permette di passare alcuni
* parametri utili per la parte grafica (client side) ad esempio il nome
* della classe nell'albero completo di numero delle istanze tra parentesi che va cambiato
* dopo una operazione che cambia il numero delle istanze
*/	
function httpGetResultP(aResultCode,parameters) {
	...
	if(aResultCode == RESULT_OK) {		
		...
		if(attr == "repository_list"){
			...
			/*var typeR = parameters.typeR;
			if(typeR == "list"){
				// si ha la lista delle implementazioni disponibili, quindi si riempie il 
				// menù con tale lista
				var list = responseXML.getElementsByTagName('Repository');
				for(var k=0; k<list.length; k++){
					if(document.getElementById(list[k].getAttribute("repName")) == null){
						var idRepository = list[k].getAttribute("repName");
						var menuPopup = document.getElementById("menuRepositoryMP");
						var menuitem = document.createElement("menuitem");
						menuitem.setAttribute("label", idRepository);
						menuitem.setAttribute("id", idRepository);
						menuitem.setAttribute("oncommand", "choseRepository(\""+idRepository+"\");");
						menuitem.setAttribute("type", "checkbox");
						menuPopup.appendChild(menuitem);
					}
				}
				// se c'è solo un repository scelgo in automatico quello
				if(list.length == 1){
					choseRepository(list[0].getAttribute("repName")); // tale funzione si trova in repository.js
				}
			} else if(typeR == "chosen"){
				// si è effettuata una scelta, quindi ora si abilitano i pulsanti per
				// interagire con l'ontologia
				
				var repID = responseXML.getElementsByTagName('Repository')[0].getAttribute("repName");
				
				repositoryChosen(repID); // tale funzione si trova in repository.js
			}*/
		}
		...
		//NScarpato 19/07/2007 add get_imports service for import panel
		   ...
		   addFromWebToMirror(msg);
			...
			var msg = treeList[0].getElementsByTagName('msg')[0];
			addFromOntologyMirror(msg);
			//NScarpato 03/09/2008 change ontology icon after mirroring of ontology
		...
		}//NScarpato 14/03/2008 change update_modify to make refresh on: class tree property tree and instance list
			...
			if(parameters.sourceType=="Class"){
				...
				for (var i = 0; i<treecellNodes.length; i++) {	//does not break because it has to change all occurrences of the given class
		...
		}else if (attr == "update_cls") {		//changes a class label (modifying the number between parentheses) when an instance is added or removed from the class
			var tree = parameters.tree;					
			...
			httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=cls&clsName=" + encodeURIComponent(clsNodeName), false);
		}
		...
		*@author NScarpato 13/04/2007*/
		...
		urlList = treeList[0].getElementsByTagName('URL');		
			for (var i = 0; i < urlList.length; i++) {
				...
				pagelbl2.setAttribute("onclick","close();");
				labelBox.appendChild(pagelbl2);
			}	
		}/**@author NScarpato 11/06/2007*
		END webPage
		**/
		/**@author NScarpato 03/03/2008
		change search panel 
		**/		
				...
				if(foundList.length>1){
					...
					window.openDialog("chrome://semantic-turkey/content/search.xul","_blank","modal=yes,resizable,centerscreen",parameters);
				}else{
					...
					param.typeName="none";
					httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=individual&request=get_directNamedTypes&indqname=" + encodeURIComponent(resName),false,param);
		...
		else if (attr == "property_dscr") {
			var rangeNode=treeList[0].getElementsByTagName('range');
			var range=rangeNode[0].getAttribute("name");
			parameters.reqparameters.objectClsName=range;
			var domainNode=treeList[0].getElementsByTagName('domain');
			var domain=rangeNode[0].getAttribute("name");
			...
			if(request=="addProperty"){
				...
				if(isRootNode=="true"){
					...
					var tc = document.createElement("treecell");
					var ti = document.createElement("treeitem");
					tc.setAttribute("label", parameters.newPropName);
					...
					tc.setAttribute("properties",parameters.propType);
					tr.setAttribute("properties",parameters.propType);									
					tr.appendChild(tc);
					var ti = document.createElement("treeitem");			
...
function httpGetResult(aResultCode) {
	...
	if(aResultCode == RESULT_OK) {			
		...
		/**@author NScarpato*
		riempie la lista delle istanze 
		**/
		else if(attr == "error") {			
			var errorNode = treeList[0].getElementsByTagName('Error')[0];
			alert("Error: " + errorNode.getAttribute("value"));
			...
			list.getElementsByTagName('listheader')[0].getElementsByTagName('listitem-iconic')[0].getElementsByTagName('label')[0].setAttribute("value","Individuals of "+myClass);
			...
			for (var i = 0; i < instancesList.length; i++) {		
				...
				//NScarpato 14/04/2008 add explicit attribute for instances list
		...
		creazione pannello ricerca 
			...
			var findType = treeList[0].getAttribute('findType');
			if(findType=="similarList"){
				var simList=treeList[0].getElementsByTagName('SimilarElement');	
				if(simList.length>1){
					var parameters = new Object();
					parameters.simList = simList;
					parameters.tree = getthetree();
					parameters.list = gettheList();
					window.openDialog("chrome://semantic-turkey/content/search.xul", "showmore", "chrome",parameters);
				}else{
					var resType = simList[0].getAttribute("resType");
					var name = simList[0].getAttribute("name");
					var typeName = simList[0].getAttribute("typeName");
					var refInstName=simList[0].getAttribute("instance_name");
					searchFocus(resType,name,typeName,refInstName,"");
				}
			}else{
				var find=treeList[0].getElementsByTagName('Find')[0];	
				var resType=find.getAttribute("resType");
				var name=find.getAttribute("name");
				var typeName=find.getAttribute("typeName");
				//NScarpato 22/05/2007 corretto comportamento della search
				var refInstName=find.getAttribute("instance_name");
				searchFocus(resType,name,typeName,refInstName,"");
			}
		}**/	
		/**Pagina delle webPage
		*@author NScarpato 21/02/2007*/
		...
		**/
		//NScarpato change annotatorProperties in template
			...
			//NScarpato 27/10/2007 change editorPanelProperties con templateandvalued
			...
			if(request=="getClsDescription" || request=="getIndDescription"){
				...
				//NScarpato 26/11/2007 change types visualization added add type and remove type
				...
				}//NScarpato 05/12/2007 add superClass list for class's editor panel
				...
				//NScarpato 26/03/2008 add title and addNewProperty option for property of instance
				...
				for (var i = 0; i < propertyList.length; i++) {					
					if (propertyList[i].nodeType == 1) {			
						...
						if(typeValue=="owl:ObjectProperty"){
							...
							typeToolbarButton.setAttribute("onclick","createAndAddPropValue('"+nameValue+"','"+typeValue+"');");
						...
						if(valueList.length>10){
							if(typeValue=="owl:ObjectProperty"){
								...
								typeToolbarButton2.setAttribute("onclick","removePropValue('list','"+nameValue+"','"+typeValue+"');");
							...
							for (var j = 0; j < valueList.length; j++) {					
								if (valueList[j].nodeType == 1) {
									...
									propButton.setAttribute("oncommand","removePropValue('"+value+"','"+nameValue+"','"+typeValue+"');");
			...
			//NScarpato 27/05/2007 add update for class tree after binding new individual for selected Class					
			var clsNodeName = treeList[0].getElementsByTagName('Class')[0].getAttribute("clsName");
			var numTotInst= treeList[0].getElementsByTagName('Class')[0].getAttribute("numTotInst");
			var treecellNodes; 
			var iconicName=clsNodeName;
		...
		//NScarpato update_cls 03/01/2007 modificato 02/03/2007 refresh lista istanze 
			...
			var newIconicName=clsNodeName+"("+numTotInst+")";
			treecellNodes = getthetree().getElementsByTagName("treecell");							
				for (var i = 0; i<treecellNodes.length; i++) {				
					if (treecellNodes[i].getAttribute("label") == iconicName ) {						
						treecellNodes[i].setAttribute("label",newIconicName);
						treecellNodes[i].setAttribute("numTotInst",""+numTotInst);
		...
		//Nscarpato update per lista istanze dopo delete 02/03/2007
			...
			if (annotations != null) {
				for (var i = 0; i < annotations.length; i++) {
					annotationsArray[i] = annotations[i].getAttribute("value");				
				}							
			}
			
			var instanceDatatypePropertyNodes = treeList[0].getElementsByTagName('Property');
			
			if (instanceDatatypePropertyNodes != null) {
				for (var i = 0; i < instanceDatatypePropertyNodes.length; i++) {
					annotationsArray[i] = instanceDatatypePropertyNodes[i].getAttribute("value");				
				}							
			}
									
			active(annotationsArray);	*/								
			//NScarpato add highlight 15/05/2008 
																			
		}//END annotations
		
		/*NScarpato 14/03/2008 messo update_modify in httpGetP
		 * else if (attr == "update_modify") {			
			var treecellNodes = getthetree().getElementsByTagName("treecell");									
			var resourceNode = treeList[0].getElementsByTagName('UpdateResource')[0];
			var targetNode;
			for (var i = 0; i<treecellNodes.length; i++) {												
				if (treecellNodes[i].getAttribute("label") == resourceNode.getAttribute("name")) {						
					alert("sto per cambiare "+resourceNode.getAttribute("name")+"con "+resourceNode.getAttribute("newname"));
					treecellNodes[i].setAttribute("label", resourceNode.getAttribute("newname"));
				}				
			}			
		}*/
...
function parsing(classNode, node,isRootNode) {				
	...
	tc.setAttribute("deleteForbidden", classNode.getAttribute("deleteForbidden"));
		...
		if (classList[i].nodeName == "Instances") {		
			...
			for (var j = 0; j < instanceNodes.length; j++) {				
				if (instanceNodes[j].nodeType == 1) {
					//_printToJSConsole("instanceNode: " + instanceNodes[j].nodeType);
					var trInst = document.createElement("treerow");
					//trInst.setAttribute("id", "treerow" + 30);
					var tcInst = document.createElement("treecell");
					//_printToJSConsole("instanceNode: " + instanceNodes[j].getAttribute("name"));
					tcInst.setAttribute("properties", "individual");
					tcInst.setAttribute("label", instanceNodes[j].getAttribute("name"));
...
/**NScarpato 18/06/2007
*This function make parsing for properties tree  
*/
...
/**NScarpato 12/07/2007
*This function make parsing for prefix namespace tree  
* 
*/
...
/**NScarpato 04/10/2007
*This function make parsing for imports tree  
* 
*/
...
/**NScarpato 23/05/2007
* This function focus result element to ontologySearch
...
function searchFocus(resType,resName,callPanel,typeName){
	...
	if(callPanel=="class" ){
		if(resType=="owl:Individual"){
			...
			httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=cls&clsName=" + encodeURIComponent(typeName), false);
			list=gettheList();
			index=0;
			...
			httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=cls&clsName=" + encodeURIComponent(resName), false);
	...
	//property	
		...
		httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=cls&clsName=" + encodeURIComponent(name), false);
	}else if(resType=="Individual"){
		var myTree = getthetree();
		if(callPanel=="class"){
			myTree.selectElementClass(myTree,typeName);
			httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=cls&clsName=" + encodeURIComponent(typeName), false);
				...
				index=0;
				while(list.getItemAtIndex(index)!=null){
					if (list.getItemAtIndex(index).label==name) {
						list.selectedIndex=index;
						list.scrollToIndex(index);
						break;		
					} 
		...
		_printToJSConsole("e' una proprieta'");
			_printToJSConsole("sto per selezionare "+name+" da "+myTree.getAttribute("id"));
			myTree.selectElementClass(myTree,name);
		}
	...
	//NScarpato 23/05/2007 if it's a annotation focus the refered instance
		...
		if(resType=="annotation"){
			alert(name+" it's a annotation in reference to instance "+refInstName);
		}
		myTree.selectElementClass(myTree,typeName);
		httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=cls&clsName=" + encodeURIComponent(typeName), false);
		list=gettheList();
		index=0;
		while(list.getItemAtIndex(index)!=null){
			if (list.getItemAtIndex(index).label==refInstName) {				
				list.selectedIndex=index;
				list.scrollToIndex(index);
				break;		
			} 
	   ...
	   */
	    ...
	    */
	    function addFromWebToMirror(msg) {
			...
			httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=get_nsprefixmappings",false,parameters);
	   ...
	   */
	    ...
	    */
	     ...
	     */
	     ...
	     /**
	      * @author NScarpato 14/03/2008
	      * addSuperClass or remove Super classes
	      * @param {xml} treeList 
	      */
	      function changeSuperClass(treeList) {
	      	...
	      	if(treeList[0].getAttribute('type')=="remove_superclass"){
	      		...
	      		//alert("Class '"+value+"' correctly added");
					...
					httpGetP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=cls&request=getClsDescription&clsName="+encodeURIComponent(parameters.sourceElementName)+"&method=templateandvalued");
	      }
	      /**
	      * @author NScarpato 23/06/2008
	      * addSuperProperty or remove Super property
	      * @param {xml} treeList 
	      */
	      function changeSuperProperty(treeList) {
	      	...
	      	if(treeList[0].getAttribute('request')=="removeSuperProperty"){
	      		...
	      		//alert("Super Property correctly added");
					...
					httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=property&request=getPropDescription&propertyQName="+encodeURIComponent(parameters.sourceElementName),false,parameters);
	      }
	     /**
	      * @author NScarpato 12/03/2008
	      * changePropValue 
			...
			for(var i = 0; i < facetsList.length; i++) {				
				if (facetsList[i].nodeType == 1 && facetsList[i].tagName=="inverseOf") {
					...
					if(valueList.length>10){
						...
						remInverseBtn.setAttribute("oncommand","removeInverseOf('"+inverseList.selectedItem.label+"');");
						...
						for(k=0; k <valueList.length; k++){
							if(valueList[k].nodeType == 1 ){
								...
								//NScarpato 15/09/2008 added remove inverseOf value Button
...
/**
 * @author NScarpato 29/04/2008
 * populateRepositoryList
 * @param {XMLResponse} treeList 
 */
 ...
 */
 function manageStartST() {
 	var treeList = responseXML.getElementsByTagName('Tree');
 	parameters.start=treeList[0].getElementsByTagName('response')[0].getAttribute("state");
 	parameters.baseuri=treeList[0].getElementsByTagName('baseuri')[0].getAttribute("state");
 	parameters.state=treeList[0].getElementsByTagName('repositoryImplementation')[0].getAttribute("state");
 	parameters.repositoryImplementation=treeList[0].getElementsByTagName('repositoryImplementation')[0].getAttribute("id");  
 }
