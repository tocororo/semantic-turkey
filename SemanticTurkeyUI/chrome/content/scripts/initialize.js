...
/**
 * @author NScarpato - ATurbati 28/04/2008 populateInitializePanel
 */
...
function populateInitializePanel() {
	...
	httpGet(
			"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=systemStart&request=listRepositories",
			false, parameters);
...
/**
 * @author NScarpato 15/04/2008 isSelected Check if the baseUri and the default
 *         Namespace of working ontolgy are selected
 */
...
function onAccept() {
	...
	_printToJSConsole("repimpl " + repositoryImplementation + "baseuri "
			+ baseuri);
	// richiesta al server per settare baseuri

	/*
	 * if(repositoryImplementation!="unavailable" && isUrl(baseuri)){
	 * httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=systemStart&request=start&baseuri="+baseuri+"&repositoryImpl="+repositoryImplementation,
	 * false); NSselected=true; close(); }else{
	 * 
	 * //alert("please type a valid URI and select a repository " + // "An
	 * example of valid URI is: http://myontology"); prompt("please type a valid
	 * URI and select a repository \n An example of valid URI
	 * is:","http://myontology");
	 *  } //close();
	 */
	while (!isUrl(baseuri)) {
		baseuri = prompt(
				"please type a valid URI and select a repository \n An example of valid URI is:",
				"http://myontology");
	}
	if (repositoryImplementation != "unavailable") {
		httpGet(
				"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=systemStart&request=start&baseuri="
						+ baseuri
						+ "&repositoryImpl="
		...
		// TODO vedere cosa fare nel caso in cui non seleziono il repository di
		// norma se c'è uno è settato di default
	}
}
/**
...
/**
 * @author NScarpato 18/04/2008 lostFocus
 */
		...
		if (changed == true) {
			...
			if (risp) {
				...
				if (nstxbox.value.endsWith('#') || nstxbox.value.endsWith('/')) {
					 ...
					 */
					httpGet(
							"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=set_baseuridnspace&baseuri="
									+ encodeURIComponent(valBase)
									+ "&namespace="
									+ encodeURIComponent(nstxbox.value), false,
							parameters);
				} else {
		...
		if (type == "base") {
			if (changed == true) {
				...
				if (risp) {
					...
					httpGet(
							"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=set_baseuri&uri="
									+ encodeURIComponent(valBase), false,
							parameters);
			...
			if (changed == true) {
				...
				if (risp) {
					if (txbox.value.endsWith('#') || txbox.value.endsWith('/')) {
						...
						parameters.ns = "none";
						httpGet(
								"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=set_defaultnamespace&namespace="
										+ encodeURIComponent(txbox.value),
								false, parameters);
					} else {
						alert("Default Namespace should end with '#' or '/'");
					}
 ...
 */
function checkEnter(e, type, txbox) { // e is event object passed from
										// function invocation
...
function startST() {
	...
	httpGetP(
			"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=systemStart&request=start",
			false, parameters);
	if (parameters.start == "negative") {
		param = Components.classes["@mozilla.org/embedcomp/dialogparam;1"]
		...
		win.openWindow(null, "chrome://semantic-turkey/content/initialize.xul",
				"initialize", "chrome,modal,centerscreen", param);
	}
}
