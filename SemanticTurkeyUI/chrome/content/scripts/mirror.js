...
function populateMirrorPanel() {
	...
	httpGetP(
			"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=administration&request=getOntologyMirror",
			false, parameters);
...
function deleteMirrorEntry() {
	try {
		...
		httpGet(
				"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=administration&request=deleteOntMirrorEntry&ns="
						+ encodeURIComponent(baseURI_FilePair.baseURI)
						+ "&file=" + encodeURIComponent(baseURI_FilePair.file),
				false);
...
/**
 * @author NScarpato 17/03/2008 removeMirrorEntryEvent
 */
...
/**
 * @author NScarpato 17/03/2008 updateMirrorEntryEvent
 */
...
function updateMirrorEntry() {
	try {
		...
		window.openDialog("chrome://semantic-turkey/content/updateMirror.xul",
...
function updateMirror() {
	...
	if (selectedUpdate.getAttribute("id") == "web") {
		httpGet(
				"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=administration&request=updateOntMirrorEntry&baseURI="
						+ encodeURIComponent(baseURI_FilePair.baseURI)
						+ "&mirrorFileName="
						+ encodeURIComponent(baseURI_FilePair.file)
						+ "&srcLoc=wbu", false);
		...
		if (newMirrorFilePath != null) {
			httpGet(
					"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=administration&request=updateOntMirrorEntry&baseURI="
							...
							+ encodeURIComponent(baseURI_FilePair.file)
							+ "&altURL="
							+ encodeURIComponent(newMirrorFilePath)
							+ "&srcLoc=walturl", false);
		}
		...
		window
				.openDialog(
						"chrome://semantic-turkey/content/updateMirrorFromLocalFile.xul",
...
function onAccept() {
	...
	httpGetP(
			"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=addFromOntologyMirror&baseuri="
					+ encodeURIComponent(namespace)
					+ "&mirrorFile="
					+ encodeURIComponent(localFile), false, parameters);
}
/**
 * NScarpato 19-09-2008 function to update mirror entry by a local file
 */
function onLocalFileAccept() {
	...
	httpGet(
			"http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=administration&request=updateOntMirrorEntry&baseURI="
					...
					+ encodeURIComponent(baseURI_FilePair.file)
					+ "&updateFilePath="
					+ encodeURIComponent(updateFilePath)
					+ "&srcLoc=lf", false);
	close();
}
