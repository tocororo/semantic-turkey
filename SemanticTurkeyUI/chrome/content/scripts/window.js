...
{  
  if (event.originalTarget instanceof HTMLDocument) {
    if (event.originalTarget.defaultView.frameElement) {
      ...
      //httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=annotation&urlPage=" + ciao, false);
            
      _printToJSConsole("doc: " + gBrowser.selectedBrowser.currentURI.spec);
      _printToJSConsole("list: " + list.length);
      
      var url = gBrowser.selectedBrowser.currentURI.spec;
      url = url.replace("&", "%26");
  	  httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=annotation&request=chkAnnotations&urlPage=" + encodeURIComponent(url));
    }
  }
}
...
function viewAnnotationOnPage() {
	_printToJSConsole("viewAnnotationOnPage");	
		...
		//findTextannotation(currentdoc, annotationValue);
		 var url = gBrowser.selectedBrowser.currentURI.spec;
     	 url = url.replace("&", "%26");
      	httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=annotation&request=getPageAnnotations&urlPage=" + encodeURIComponent(url));
}
...
/*function findText(doc) {
      ...
      var regex = new RegExp("[ a-z0-9]*sufficiente[ a-z0-9]*");
      for (var i = 0; i < childNodes.length; i++) {
      	//_printToJSConsole("name: " + childNodes[i].nodeName);
	if (childNodes[i].nodeType == 1) {
		findText(childNodes[i]);
	}
      	if (childNodes[i].nodeType == 3) {
		//_printToJSConsole("value: " + childNodes[i].nodeValue);
		if (regex.test(childNodes[i].nodeValue)) {
			_printToJSConsole("YYYYYYYYYEEEES!");							
			//childNodes[i].nodeValue = 
			var temp = childNodes[i].nodeValue.split("sufficiente");			
			var parentNode = childNodes[i].parentNode;
			_printToJSConsole("temp0: " + temp[0] + "****");						
			_printToJSConsole("temp1 " + temp[1]);										
			parentNode.removeChild(childNodes[i]);		
			var newSpan = document.createElement("div");
			newSpan.setAttribute("style", "background-color:yellow");
			var textNode1 = document.createTextNode(temp[0]); 
			var textNode2 = document.createTextNode(temp[1]);  
			var textNode3 = document.createTextNode("sufficiente");   
			parentNode.appendChild(textNode1);
			newSpan.appendChild(textNode3);	
