...
/**funzione che attiva il focus sull'elemento scelto
...
function onAccept() {			
	...
	httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=individual&request=get_directNamedTypes&indqname=" + encodeURIComponent(resName),false,parameters);
...
function initList(foundList,callPanel) {			
	for (var i = 0; i < foundList.length; i++) {								
			...
			/*if(type=="annotation" && callPanel=="class" ){
				var instance_name = foundList[i].getAttribute("instance_name");
        		lsti.setAttribute("label",foundList[i].getAttribute("name"));
				lsti.setAttribute("refInstName",foundList[i].getAttribute("instance_name"));				
				lbl.setAttribute("value","annotation ");
				lbl2 = document.createElement("label");
        		lbl2.setAttribute("value",name);  
        		lbl2.setAttribute("style","font-style: italic;");
				lbl3 = document.createElement("label");
				lbl3.setAttribute("value"," referring to ");
				img2=document.createElement("image");
				...
				lci.appendChild(img);
				lci.appendChild(lbl);
				lci.appendChild(lbl2);
				lci.appendChild(lbl3);
				lci.appendChild(img2);
				lci.appendChild(lbl4);
				lsti.appendChild(lci);
				myList.appendChild(lsti);
			}else if(type=="Instance"){
				var typeName = foundList[i].getAttribute("name");
			 	if(typeName=="Property" ){
			 		if(callPanel=="property"){ 
			 			img.setAttribute("src","images/prop.png");
			 		...
			 		if(callPanel=="class"){ 
			 			img.setAttribute("src","images/individual.png");
			 			name = type+" "+simList[i].getAttribute("name");
...
function listdblclick(event) {
	    ...
	    if(event.target.getAttribute("typeName")=="Property"){
		    ...
		    if(event.target.getAttribute("resType")=="Instance"){
		    	var callPanel=getCallPanel();
		    	searchFocus(event.target.getAttribute("resType"),event.target.getAttribute("label"),event.target.getAttribute("typeName"),"",callPanel);
		    }
		    parameters.sourceElementName = event.target.getAttribute("label");
		    parameters.sourceParentElementName = event.target.getAttribute("typeName");
		    parameters.list=gettheList();
		    parameters.tree=getthetree();
			parameters.domain="";
			parameters.range="";
...
OntologySearch.search = function(element)  {	
	if (element != '') {
		...
		httpGetP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=ontologySearch&request=searchOntology&inputString=" + encodeURIComponent(element)+"&types=clsNInd",false,parameters);      	
...
OntologySearch.searchProperty = function(element)  {	
	if (element != '') {
		...
		httpGetP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=ontologySearch&request=searchOntology&inputString=" + encodeURIComponent(element)+"&types=property",false,parameters);      	
	}
};