...
function addImport() {
		...
		selectLabel=document.getElementById("import").selectedItem.getAttribute("label");
		if(selectItem==0){
			...
			if(selectItem==4){
				...
				window.openDialog("chrome://semantic-turkey/content/selectFileFromMirror.xul","_blank","modal=yes,resizable,centerscreen",parameters);
			}else{
				window.openDialog("chrome://semantic-turkey/content/addImport.xul","_blank","modal=yes,resizable,centerscreen",parameters);
			}
	    ...
	    try{
    	  var currentelement = tree.treeBoxObject.view.getItemAtIndex(tree.currentIndex);
 		  ...
 		  return;
	    	...
	    	httpGetP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=removeImport&uri="+encodeURIComponent(uri),false,parameters);     	
}
/**
 * addPrefix
 * 
 */
 function addPrefix() {
	...
	parameters.namespaceTree=getNamespaceTree();
	parameters.importsBox=document.getElementById("importsBox");
	window.openDialog("chrome://semantic-turkey/content/add_prefix.xul","_blank","modal=yes,resizable,centerscreen",parameters);
 }
 ...
 function removePrefix(){
	...
	if(explicit=="true"){
    	...
    	parameters.importsTree=tree;
  		parameters.namespaceTree=getNamespaceTree();
  		parameters.importsBox=document.getElementById("importsBox"); 
		httpGetP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=remove_nsprefixmapping&namespace="+encodeURIComponent(namespace),false,parameters);
	}else{
 ...
 function changePrefix(){
	...
	//if(explicit=="true"){
	    ...
	    parameters.importsBox=document.getElementById("importsBox");
		window.openDialog("chrome://semantic-turkey/content/change_prefix.xul","_blank","modal=yes,resizable,centerscreen",parameters);
	/*}else{
		alert("You cannot change this prefix, it's a prefix that belongs to the top ontology!");
	} */  		
 }
...
function populateImportPanel() {
	...
	httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=get_defaultnamespace",false,parameters);
	httpGet("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=get_baseuri",false,parameters);
  ...
  */
  function mirrorOntology() {
	...
	parameters.namespaceTree=getNamespaceTree();
  	...
  	httpGetP("http://127.0.0.1:1979/semantic_turkey/resources/stserver/STServer?service=metadata&request=mirrorOntology&baseuri="+encodeURIComponent(uri)+"&mirrorFile="+encodeURIComponent(mirrorname),false,parameters); 
  }
