...
function repositoryChosen(idRepository) {
	...
	// _printToJSConsole("in menuRepositoryMP ci sono "+list.length+"
...
function choseRepository(idRepository) {
	...
	// _printToJSConsole("*****2 choseRepository: "+idRepository); // da
	// cancellare
	httpGetP(
...
function isRepLoaded() {
	...
	// _printToJSConsole("document.getElementById(\"viewSageSidebar\"):
	...
	if (document.getElementById("visualization2") != null) {
		...
		// _printToJSConsole("mainWindow.getElementById(\"viewSageSidebar\"):
