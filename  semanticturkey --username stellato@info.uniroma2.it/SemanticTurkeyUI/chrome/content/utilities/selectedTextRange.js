	...
	function ahy_EndSelect(event) {
		if (event.altKey) {
			...
			alert("this.ahySelectionString : " + this.ahySelectionString
					+ "\n this.ahySelection.anchorOffset : "
					+ this.ahySelection.anchorOffset
					+ "\n this.ahySelection.anchorOffset : "
					+ this.ahySelection.anchorOffset);
		}
...
function ahy_DoOnLoad() {
	if (window.getSelection) {
		var blnRangeImplemented = document.implementation.hasFeature("Range",
				"2.0");
		if (blnRangeImplemented) {
			objSelectedTextRange = new ahyObjSelectedTextRange();
			document.addEventListener("mousedown",
					objSelectedTextRange.ahy_StartSelect, false);
			document.addEventListener("mouseup",
					objSelectedTextRange.ahy_EndSelect, false);
		} else {
