...
function listDragGesture(event) {
	 ...
	 *  // Only allow dragging of treeitems below main // if
	 * (tree.view.getIndexOfItem(treeitem) <= 1) { alert("nodrag"); return } //
...
function listdblclick(event) {
	...
	// parameters.deleteForbidden =
	// event.target.getAttribute("deleteForbidden");
...
//vedere se in enrich crea problemi il fatto che è stata spostata in browser-overlay.js
function listDragDrop(event) {
	var prefs = Components.classes["@mozilla.org/preferences-service;1"]
		...
		AnnotFunctionList[defaultAnnotFun][2](event,this.document);
...
/**
 * @author NScarpato 26/03/2008 show or hidden contextmenu's items in particular
 *         the remove item that it's shown only if the ontology it's root
 *         ontology showHideItemsList
 */
