...
art_semanticturkey.init = function() {
	var treeList = document.getElementsByTagNameNS("http://semanticturkey.uniroma2.it/xmlns/widget#", "conceptTree");
	
	for (var i = 0 ; i < treeList.length ; i++) {
		treeList[i].conceptScheme = art_semanticturkey.STRequests.Projects.getProjectProperty("skos.selected_scheme", null).getElementsByTagName("property")[0].getAttribute("value");		
	
		treeList[i]._view.canDrop = function(index, orientation, dataTransfer) {
			if (index != -1 && orientation == 0) {
				return true;
			}
		};
		
		var bound = treeList[i];
		
		treeList[i]._view.drop = function(index, orientation, dataTransfer) {
			if (index == -1 || orientation != 0) {
				return;
			}
				...
				var event = {target : 
	                {
	                    ...
	                    getAttribute : function(attrName) {
	                    	if (attrName == "label") {
	                    		return bound._view.visibleRows2[index].id;
	                    	}
	...
	stEventArray.addEventListenerToArrayAndRegister("projectPropertySet", function(eventId, projectPropertySetObj) {
		if (projectPropertySetObj.getPropName() == "skos.selected_scheme") {
			for (var i = 0 ; i < treeList.length ; i++) {
				treeList[i].conceptScheme = projectPropertySetObj.getPropValue();		
			}
		}
	}, null);
