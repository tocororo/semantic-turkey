...
function buttonOkListener() {
	try{
		var concepts = window.arguments[0].concepts; //passed parameter
		...
		for (var i=0; i<checkedElement.length; i++){
			for (var j=0; j<concepts.length; j++){
				art_semanticturkey.STRequests.Property.addExistingPropValue(concepts[j], "skos:inScheme", checkedElement[i], "resource");
			}
		}
		alert("Changes done!");
	} catch (e) {
