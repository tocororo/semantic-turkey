...
public abstract class ConfigurationSupport {
	...
	public static <CONFTYPE extends Configuration> CONFTYPE loadConfiguration(
		...
		Bundle bundle = FrameworkUtil.getBundle(AopUtils.getTargetClass(configurationManager));
		BundleContext bundleContext = bundle.getBundleContext();
		ServiceReference sr = bundleContext.getServiceReference(ExtensionPointManager.class.getName());
		ExtensionPointManager exptManager = (ExtensionPointManager) bundleContext.getService(sr);
		try {
			return STPropertiesManager.loadSTPropertiesFromYAMLFiles(configBaseClass, true, exptManager,
					configFile);
		} finally {
