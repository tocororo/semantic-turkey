...
public class StatementCollector {
	...
	public final static ARTResource INFERENCE_GRAPH = VocabUtilities.nodeFactory
	...
	private Set<ARTStatement> processedStatements = new HashSet<ARTStatement>();
	...
	public Set<ARTStatement> getStatements(ARTResource subject, ARTURIResource predicate, ARTNode object) {
		return Sets.filter(getStatements(),
				StatementWithAnyOfGivenComponents_Predicate.getFilter(subject, predicate, object));
	}

	public Set<ARTStatement> getStatements(ARTResource subject, ARTURIResource predicate, ARTNode object,
			boolean excludeProcessed) {
		Set<ARTStatement> filteredStatements = Sets.filter(getStatements(),
				StatementWithAnyOfGivenComponents_Predicate.getFilter(subject, predicate, object));

		if (excludeProcessed) {
			return Sets.difference(filteredStatements, processedStatements);
		} else {
	...
	public boolean hasStatement(ARTResource subj, ARTURIResource pred, ARTNode obj, ARTResource graph) {
		for (ARTStatement stmt : Collections2.filter(getStatements(),
				StatementWithAnyOfGivenComponents_Predicate.getFilter(subj, pred, obj))) {
	...
	public void markAllStatementsAsProcessed(Collection<ARTStatement> stmts) {
		processedStatements.addAll(stmts);
	}

	public void markStatementAsProcessed(ARTStatement stmt) {
		processedStatements.add(stmt);
	}

	public boolean isStatementProcessed(ARTStatement stmt) {
		return processedStatements.contains(stmt);
	}
