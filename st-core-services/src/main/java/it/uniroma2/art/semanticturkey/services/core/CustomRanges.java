...
//				System.out.println("Adding triple:\nS:\t"+subject.getNominalValue()+"\nP:\t"+predicate.getNominalValue()+"\nO:\t"+artNode.getNominalValue());
	...
	public Response getReifiedResDescription(ARTURIResource resource, ARTURIResource predicate) 
		...
		for (CustomRangeEntryGraph creGraph : crEntries){
			...
			for (String pred : predicateList){
				ARTNodeIterator itValues = rdfModel.listValuesOfSubjPredPair(resource, rdfModel.createURIResource(pred), false, NodeFilters.ANY);
				if (itValues.hasNext()){
					...
					while (itValues.hasNext()){
						...
						objectElem.setAttribute("value", value.getNominalValue());
					}
				}
			}
		}
	...
	/**
	 * Tells whether or not the given property has a CustomRange 
	 * @param property
	 * @return
	 * @throws ModelAccessException
	 */
	@GenerateSTServiceController
	public Response hasCustomRange(String property) throws ModelAccessException{
//		CustomRangeProvider crProvider = new CustomRangeProvider();
		boolean exists = crProvider.existsCustomRangeForProperty(getOWLModel().expandQName(property));
		return createBooleanResponse(exists);
	}
