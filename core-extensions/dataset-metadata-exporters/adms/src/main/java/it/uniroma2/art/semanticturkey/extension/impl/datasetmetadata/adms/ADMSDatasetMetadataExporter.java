...
public class ADMSDatasetMetadataExporter implements DatasetMetadataExporter {
	...
	private final ADMSDatasetMetadataExporterFactory factory;
	...
	public Model produceDatasetMetadata(Project project, RepositoryConnection conn, IRI dataGraph)
		...
		try {
			try (RepositoryConnection metadataConnection = tempMetadataRepository.getConnection()) {
				 ...
				 * project);
				 */
				ADMSDatasetMetadataExporterSettings pluginSettings = factory.getProjectSettings(project);
				...
				metadataModel
						.setNamespace(new SimpleNamespace("wdrs", "http://www.w3.org/2007/05/powder-s#"));
				...
				if (asset_spatial != null && !asset_spatial.isEmpty()) {
					...
					metadataModel
							.add(valueFactory.createStatement(asset_spatial_iri, RDF.TYPE, DCTERMS.LOCATION));
					metadataModel
							.add(valueFactory.createStatement(assetIRI, DCTERMS.SPATIAL, asset_spatial_iri));
				}
				...
				if (asset_landingPage != null && !asset_landingPage.isEmpty()) {
					...
					metadataModel.add(
							valueFactory.createStatement(asset_landingPage_iri, RDF.TYPE, FOAF.DOCUMENT));
					metadataModel.add(
							valueFactory.createStatement(assetIRI, DCAT.LANDING_PAGE, asset_landingPage_iri));
				}

				String asset_landingPage_title = pluginSettings.asset_landingPage_title;
				if (asset_landingPage_iri != null && asset_landingPage_title != null
						&& !asset_landingPage_title.isEmpty()) {
				...
				if (asset_describedBy != null && !asset_describedBy.isEmpty()) {
					...
					metadataModel.add(
							valueFactory.createStatement(asset_describedBy_iri, RDF.TYPE, FOAF.DOCUMENT));
					metadataModel.add(valueFactory.createStatement(assetIRI,
							valueFactory.createIRI("http://www.w3.org/2007/05/powder-s#describedBy"),
							asset_describedBy_iri));
				}

				String asset_describedBy_title = pluginSettings.asset_landingPage_title;
				if (asset_describedBy_iri != null && asset_describedBy_title != null
						&& !asset_describedBy_title.isEmpty()) {
				...
				if (asset_page != null && !asset_page.isEmpty()) {
					...
					metadataModel.add(valueFactory.createStatement(assetIRI, FOAF.PAGE, asset_page_iri));
				}
				...
				if (asset_identifier_iri != null && asset_identifier_notation != null
						&& !asset_identifier_notation.isEmpty()) {
				...
				if (asset_identifier_iri != null && asset_identifier_schemeAgency != null
						&& !asset_identifier_schemeAgency.isEmpty()) {
				...
				if (asset_temporal != null && !asset_temporal.isEmpty()) {
					...
					metadataModel.add(valueFactory.createStatement(asset_temporal_iri, RDF.TYPE,
							DCTERMS.PERIOD_OF_TIME));
					metadataModel.add(
							valueFactory.createStatement(assetIRI, DCTERMS.TEMPORAL, asset_temporal_iri));
				}

				String asset_temporal_startDate = pluginSettings.asset_temporal_startDate;
				if (asset_temporal_iri != null && asset_temporal_startDate != null
				...
				if (asset_temporal_iri != null && asset_temporal_endDate != null
				...
				if (asset_language != null && !asset_language.isEmpty()) {
					...
					metadataModel.add(valueFactory.createStatement(asset_language_iri, RDF.TYPE,
							DCTERMS.LINGUISTIC_SYSTEM));
					metadataModel.add(
							valueFactory.createStatement(assetIRI, DCTERMS.LANGUAGE, asset_language_iri));
				}
				...
				if (asset_status != null && !asset_status.isEmpty()) {
					...
					metadataModel.add(
							valueFactory.createStatement(assetIRI, ADMSFragment.STATUS, asset_status_iri));
				}
				...
				if (asset_interoperability_level != null && !asset_interoperability_level.isEmpty()) {
					IRI asset_interoperability_level_iri = valueFactory
							.createIRI(asset_interoperability_level);
					metadataModel.add(valueFactory.createStatement(asset_interoperability_level_iri, RDF.TYPE,
							SKOS.CONCEPT));
					metadataModel.add(valueFactory.createStatement(assetIRI,
							ADMSFragment.INTEROPERABILITYLEVEL, asset_interoperability_level_iri));
				}
				...
				if (asset_type != null && !asset_type.isEmpty()) {
					...
					metadataModel.add(valueFactory.createStatement(assetIRI, DCTERMS.TYPE, asset_type_iri));
				}
				...
				if (asset_includeAsset != null && !asset_includeAsset.isEmpty()) {
					...
					metadataModel.add(valueFactory.createStatement(asset_includeAsset_iri, RDF.TYPE,
				...
				if (asset_sample != null && !asset_sample.isEmpty()) {
					...
					metadataModel.add(valueFactory.createStatement(asset_sample_iri, RDF.TYPE,
							ADMSFragment.ASSETCLASS));
					metadataModel.add(
				...
				if (asset_prev != null && !asset_prev.isEmpty()) {
					...
					metadataModel.add(
							valueFactory.createStatement(asset_prev_iri, RDF.TYPE, ADMSFragment.ASSETCLASS));
					metadataModel
							.add(valueFactory.createStatement(assetIRI, ADMSFragment.PREV, asset_prev_iri));
				}
				...
				if (asset_last != null && !asset_last.isEmpty()) {
					...
					metadataModel.add(
							valueFactory.createStatement(asset_last_iri, RDF.TYPE, ADMSFragment.ASSETCLASS));
					metadataModel
							.add(valueFactory.createStatement(assetIRI, ADMSFragment.LAST, asset_last_iri));
				}
				...
				if (asset_next != null && !asset_next.isEmpty()) {
					...
					metadataModel.add(
							valueFactory.createStatement(asset_next_iri, RDF.TYPE, ADMSFragment.ASSETCLASS));
					metadataModel
							.add(valueFactory.createStatement(assetIRI, ADMSFragment.NEXT, asset_next_iri));
				}
				...
				if (asset_relation != null && !asset_relation.isEmpty()) {
					...
					metadataModel.add(valueFactory.createStatement(asset_relation_iri, RDF.TYPE,
							ADMSFragment.ASSETCLASS));
					metadataModel.add(
							valueFactory.createStatement(assetIRI, DCTERMS.RELATION, asset_relation_iri));
				}
				...
				if (asset_distribution != null && !asset_distribution.isEmpty()) {
					...
					metadataModel.add(valueFactory.createStatement(asset_distribution_iri, RDF.TYPE,
							ADMSFragment.ASSETDISTRIBUTIONCLASS));
				...
				if (asset_publisher != null && !asset_publisher.isEmpty()) {
					...
					metadataModel
							.add(valueFactory.createStatement(asset_publisher_iri, RDF.TYPE, FOAF.AGENT));
					metadataModel.add(
							valueFactory.createStatement(assetIRI, DCTERMS.PUBLISHER, asset_publisher_iri));
				}

				String asset_publisher_name = pluginSettings.asset_publisher_name;
				if (asset_publisher_iri != null && asset_publisher_name != null
						&& !asset_publisher_name.isEmpty()) {
				...
				if (asset_publisher_iri != null && asset_publisher_type != null
						&& !asset_publisher_type.isEmpty()) {
					IRI asset_publisher_type_iri = valueFactory.createIRI(asset_publisher_type);
					metadataModel.add(
							valueFactory.createStatement(asset_publisher_type_iri, RDF.TYPE, SKOS.CONCEPT));
					metadataModel.add(
							valueFactory.createStatement(assetIRI, DCTERMS.TYPE, asset_publisher_type_iri));
				}
				...
				if (asset_distribution_iri != null) {
					...
					if (distribution_description != null && !distribution_description.isEmpty()) {
						metadataModel
								.add(valueFactory.createStatement(asset_distribution_iri, DCTERMS.DESCRIPTION,
					...
					if (distribution_downloadUrl != null && !distribution_downloadUrl.isEmpty()) {
						metadataModel.add(valueFactory.createStatement(asset_distribution_iri,
								DCAT.DOWNLOAD_URL, valueFactory.createIRI(distribution_downloadUrl)));
					}
					...
					if (distribution_licence != null && !distribution_licence.isEmpty()) {
						...
						metadataModel.add(valueFactory.createStatement(asset_distribution_iri,
								DCTERMS.LICENSE, distribution_licence_iri));
					}

					String distribution_licence_title = pluginSettings.distribution_licence_title;
					if (distribution_licence_iri != null && distribution_licence_title != null
							&& !distribution_licence_title.isEmpty()) {
						metadataModel
								.add(valueFactory.createStatement(distribution_licence_iri, DCTERMS.TITLE,
					...
					if (distribution_licence_iri != null && distribution_licence_description != null
							&& !distribution_licence_description.isEmpty()) {
						metadataModel.add(valueFactory.createStatement(distribution_licence_iri,
								DCTERMS.DESCRIPTION,
					...
					if (distribution_licence_iri != null && distribution_licence_type != null
							&& !distribution_licence_type.isEmpty()) {
						IRI distribution_licence_type_iri = valueFactory.createIRI(distribution_licence_type);
						metadataModel.add(valueFactory.createStatement(distribution_licence_type_iri,
					...
					if (distribution_format != null && !distribution_format.isEmpty()) {
						metadataModel.add(valueFactory.createStatement(distribution_licence_iri,
								DCTERMS.FORMAT, valueFactory.createIRI(distribution_format)));
					}
					...
					if (distribution_media_type != null && !distribution_media_type.isEmpty()) {
						metadataModel.add(valueFactory.createStatement(distribution_licence_iri,
								DCAT.MEDIA_TYPE, valueFactory.createIRI(distribution_media_type)));
					}

				}
				/*
				 * String = pluginSettings.; if(!=null && !.isEmpty()){
				 * metadataModel.add(valueFactory.createStatement(assetIRI, , valueFactory.createLiteral()));
				 * }
				 */
	...
	public Map<Scope, Settings> importFromMetadataRegistry(Project project) throws NoSuchDatasetMetadataException, MetadataRegistryStateException {
		STMetadataRegistryBackend registryBackend = factory.getMetadataRegistryBackend();

		@Nullable IRI mdrIRI = registryBackend.findDatasetForProject(project);

		ADMSDatasetMetadataExporterSettings settings = new ADMSDatasetMetadataExporterSettings();

		if (mdrIRI != null) {
			DatasetMetadata datasetMetadata = registryBackend.getDatasetMetadata(mdrIRI);

			datasetMetadata.getTitle().ifPresent(s -> settings.asset_title = s);
			datasetMetadata.getDescription().ifPresent(s -> settings.asset_description = s);
		}
